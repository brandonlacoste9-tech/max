@echo off
chcp 65001 >nul
title Instalador CADET - Español (Venezuela)
color 0A
...
echo   Nota: Considera usar una conexión WiFi estable para la descarga.
echo.
...
echo   Puedes saltar esto y descargar manualmente si la conexión es lenta.
...
echo   "region": "ve",>> "%INSTALL_DIR%\config.json"
echo   "language": "es-ve",>> "%INSTALL_DIR%\config.json"
...
echo.
echo IMPORTANTE - Precios en Venezuela:
echo   Todos los precios se muestran en USD (dólares americanos)
echo   para mantener estabilidad ante la situación económica.
echo   Aceptamos Zelle, PayPal y criptomonedas.
echo.
echo Para comenzar:
