@echo off
chcp 65001 >nul
title Instalador CADET - Español (Argentina)
color 0A
...
if %FREE_SPACE% LSS 16106127360 (
    echo ATENCIÓN: Capaz no tenés suficiente espacio en disco.
    echo Requerido: 15 GB libres
    echo Disponible: %FREE_SPACE% bytes
    echo.
    choice /C SN /M "¿Continuar igual"
...
set RAM_GB=%RAM:~0,-9%
if %RAM_GB% LSS 8 (
    echo ATENCIÓN: CADET funciona mejor con 8 GB+ de RAM. Tenés %RAM_GB% GB.
    echo.
    choice /C SN /M "¿Continuar igual"
    if errorlevel 2 exit /b 1
)
...
echo Esto puede tardar unos minutos según tu velocidad de internet...
echo.
...
powershell -Command "Invoke-WebRequest -Uri 'https://cdn.cadet.ai/models/cadet-base-v2.gguf' -OutFile '%INSTALL_DIR%\models\base.gguf' -UseBasicP" 2>nul || (
    echo ATENCIÓN: No se pudo descargar el modelo de IA. Lo vas a tener que descargar manualmente.
)
...
echo   "region": "ar",>> "%INSTALL_DIR%\config.json"
echo   "language": "es-ar",>> "%INSTALL_DIR%\config.json"
echo   "currency": "ARS",>> "%INSTALL_DIR%\config.json"
...
echo.
echo Para empezar:
echo   1. Hacé doble clic en el ícono de CADET en tu escritorio
echo   2. O abrí el Menú Inicio ^> CADET
echo   3. Escribí 'cadet' en cualquier terminal
echo.
echo ¿Necesitás ayuda? Visitá: https://cadet.ai/ayuda
echo Correo de soporte: ayuda@cadet.ai
echo.
echo Presioná cualquier tecla para iniciar CADET...
pause >nul
